...
### 版本号纪律(用户指令:下一次版本号该更新就更新)

- **tag 一旦推过就不再删除重打**:同号重打 = 同一版本号对应不同代码,
  用户装机报障时无法对号(v1.4.27 曾被重打 8 次,教训)
- CI 挂了/漏了改动 → **递增新 tag**(旧 draft 删掉,新版本重来);
  删除已推 tag 仅允许在"从未被任何人下载过"的一分钟内
- 诊断报告已带 version 戳,版本唯一性是远程排障的前提
