...
**Autonomous behaviors**: idle breathing & blinking / walking / relocating flights / **diving to catch fish** (splash + fish + swallow) / singing / low darting runs / perching & watching / sunbathing / napping — picked randomly every 3.5–7s. It poops after eating (physics: lands on window tops or the Dock), occasionally mid-air.

**Interactions**: click → chirp + shy hearts; drag to move (release mid-air and it flies off; drop near a window top / Dock within ±70px and it snaps on). The bird follows a window it perches on (incremental follow), and your window-dragging always takes priority over preset behaviors. A branch appears *before* the bird lands whenever it stops in mid-air.

**6 themes** (flat / clay / pixel / neon / ink / watercolor) applied to *everything* — bird, shadow, branch, poop, splashes, notes, zzz, sun, cracks. Settings window: theme, activity, animation speed, sound.

**Chirps** are synthesized from real kingfisher recordings (FFT-analyzed downslur, 4 variants).

**Bilingual** (中文 / English): follows your system language by default; switch anytime from the menu bar.

**Robustness**: sleep/wake handled end-to-end (timers suspended before sleep, wake clears stale effects), window-leak fixes, CPU watchdog with circuit breaker, multi-display support.

## Build (macOS 13+)

```bash
./build.sh        # SwiftPM release build → KingfisherPet.app → launch
```

## Windows

A Tauri 2 port lives in `windows/` (see its own notes; core behaviors + themes + tray are ported).
