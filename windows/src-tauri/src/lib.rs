...
fn build_menu(app: &tauri::AppHandle<tauri::Wry>) -> MenuResult {
    ...
    // ── 动作区(平铺,对应 Mac 菜单上半)──
    // 动作项始终可用:隐藏时点击 = 前端 withVisible 自动先破壳显示再执行
    // (比置灰优雅:无延迟、一步到位;置灰方案有"打开时快照"的延迟观感)
    ...
    let feed = MenuItem::with_id(app, "feed", t("喂条鱼", "Feed a Fish"), true, None::<&str>)?;
