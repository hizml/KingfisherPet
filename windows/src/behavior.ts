...
function currentHour(): number {
  const o = parseInt(localStorage.getItem("kf_hour_override") ?? "", 10);
...
async function think() {
  ...
  const actions: Array<() => void> = [
    ...
    () => startWatch(),
    () => startSun(),
    () => { if (settings.peckScreen) startPeck(); else { enter("idle"); scheduleThink(); } },   // 啄屏可关
    () => startPerchWindow(),
    () => startPoop(),
  ];
  ...
  for (let i = 0; i < actions.length; i++) {
    acc += widths[i] * k;
    if (r < acc) { actions[i](); return; }
  }
  startSleep();   // 兜底桶:白天≈7.2(带宽和 61,/62 余量进兜底),深夜被昼夜系数放大;与 Mac 同公式同分布
}
